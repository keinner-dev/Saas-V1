...
export const DataTable = <Data, Value > ({
    ...
    return (
        <div className="max-mg:-mx-4 max-lg:-mx-8">
            <div className="flex gap-4 p-6 max-lg:flex-col lg:justify-between lg:py-3">
                <ToggleGroup type="single" variant="outline" value={filter} onValueChange={(value: Filter) => setFilter(value)}>
                ...
                <div className="flex gap-3">
                    <InputGroup>
                        <InputGroupInput placeholder="Search" value={table.getColumn('name')?.getFilterValue() as string ?? ''} onChange={(event) => {table.getColumn('name')?.setFilterValue(event.currentTarget.value)}} />
        ...
        <div className="flex gap-3 justify-between items-center border-t py-3 px-6">
            ...
            <Button variant="outline" className="max-md:-order-1" onClick={() => table.previousPage()} disabled={!table.getCanPreviousPage()}>
                ...
                <span className="max-md:hidden">Previus</span>
            </Button>
